...
OCR_MODEL = "../model/ppocrv4_rec_fp16.rknn"
OCR_TEST_IMAGE = OCR_DIR.parent / "model" / "test.png"

RESULT_DIR = ROOT / "ocr_results_rknn"
...
def candidate_match(predicted_text: str, confidence: float):
    # 去掉首尾空格及OCR可能产生的内部空格
    normalized_text = predicted_text.strip().replace(" ", "")
    ...
    # 空结果、公共后缀和低置信度保持拒识
    if normalized_text in GENERIC_TEXTS:
        decision = "unknown"
