...
# 旧脚本使用 ws-api.xfyun.cn 参与签名；当前官方文档使用
# tts-api.xfyun.cn。依次尝试两种方式，以兼容原有账号和当前接口。
...
def main():
    try:
        ...
        print(f"✅ 提示音合成完毕：{OUTPUT_FILE}")
        print(f"📦 PCM数据长度：{len(pcm_data)} 字节")
        return 0
    except Exception as error:
        print(f"❌ 音频合成失败：{error}", file=sys.stderr)
        return 1
