...
#include "ucarmain2026/getpose_server.h"//获取yaw以便在== 3的分支中筛选雷达点
...
private:
    ...
    //雷达第一个点是-180度，然后逆时针旋转,第175个点才是0度
        ...
        { // 拣货区对准后绕行用
            ucarmain2026::getpose_server pose_srv;
...
public:
    LidarProcessor() : nh_("~"),tf_listener_(tf_buffer_) {
        ...
        getpose_client_ = nh_global_.serviceClient<ucarmain2026::getpose_server>("/getpose_server");
