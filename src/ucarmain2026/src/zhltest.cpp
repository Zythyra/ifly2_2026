...
//     ros::ServiceClient client_find_board = nh.serviceClient<ucarmain2026::lidar_process>("/lidar_process/lidar_process");
//     ucarmain2026::lidar_process where_board;
//     client_find_board.waitForExistence();
//     ROS_INFO("等待坐标获取服务中---");
//     ros::ServiceClient poseget_client = nh.serviceClient<ucarmain2026::getpose_server>("getpose_server");
//     ucarmain2026::getpose_server pose_result;
...
for(int i = 0; i < 9; i++){
    ...
    // play_audio(voice[5][8]);
    // play_audio(voice[5][0]);
    // play_audio(voice[5][1]);
    // play_audio(voice[2][2]);
    // play_audio(voice[3][0]);
    // play_audio(voice[3][1]);
    // play_audio(voice[1][5]);
