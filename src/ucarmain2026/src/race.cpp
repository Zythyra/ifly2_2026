 ...
 *
 * V17：现实目标停靠后快速恢复巡检
 *      先纯旋转到巡检方向 -> 停稳后计算最短法向距离
 *      -> vx=0低速vy横移回巡检线 -> 3cm接管保护。
 */
...
// V21_SEMANTIC_VALIDATION_SYNC_20260819：
// race.cpp 语义结果校验同步 race_2.cpp：
// real/sim 仅允许 food/daily/electronic，且二者不能相同；
// 非法时清空结果并重新录音；其余流程保持不变。
...
private:
    ...
    // V15：现实目标停靠后的安全回巡检逻辑
    //
    // 旧 retreatToPatrolLine() 的“沿当前车头后退即可回线”假设已经删除。
    //
    // 新顺序严格固定：
    //   A. 先纯旋转到 segment.travel_yaw；
    //   B. 完全停稳后重新读取 map 位姿；
    //   C. 此时才计算当前位置到巡检直线的最短法向距离；
    //   D. vx=0，仅低速vy横移回线，并用小wz保持航向；
    //   E. 横向误差稳定后做3cm最终接管保护。
    // ------------------------------------------------------------------------
...
// ============================================================================
// V19：最终红绿灯 RKNN + 巡线模块
// 来源：当前实测 traffic_light.cpp。
// YOLOv5模型映射：0=LEFT，1=RIGHT，2=STRAIGHT（实车测试已确认）。
// 路线保持：LEFT->/line2_left，STRAIGHT->/line_right，RIGHT->/line2_right。
...
int main(int argc, char** argv) {
    ...
    simulation_target_pub =
        nh.advertise<std_msgs::String>("/detected_target", 1, false);
    ...
    tts_trim_silence_threshold =
        max(0, min(32767, tts_trim_silence_threshold));
    tts_trim_keep_ms = max(0.0, tts_trim_keep_ms);
    tts_crossfade_ms = max(0.0, tts_crossfade_ms);
        ...
        switch (current_state) {
            ...
            case SEMANTIC_PARSING:
            {
                ...
                if (
                    ...
                    // 与 race_2.cpp 完全相同的语义类别合法性校验：
                    // real / sim 都只能是 food / daily / electronic，
                    // 且两个目标类别不能相同。
                    ...
                    // race.cpp 没有 race_2 的坡道状态，
                    // 所以合法语义确认后保持原 race.cpp 流程，
                    // 继续进入 NAVIGATING。
                    current_state =
                        NAVIGATING;
                } else {
    ...
    ROS_INFO("语音、二维码、V14.9找板停靠、仿真通信、分阶段播报、红绿灯与巡线流程全部结束");
    return 0;
}