...
bool find_other_coner_edge(Mat gray_img,Point& right_edge_point,Mat& visualizeImg){//拐角的特征是这一行有下一行没
    bool is_now_white = false;
    vector<Point> maybe_right_point;
    ...
    for (int i = 2; i <638; i++) {
        ...
        if (!is_now_white && gray_img.at<uchar>(75, i) == 255) {
            maybe_right_point.emplace_back(i, 75);
    ...
    if(second>0.1){
        ...
        if(racetracks[first_index].points[0].x<racetracks[second_index].points[0].x){
            best_idx = second_index;
        }
        else{
            best_idx = first_index;//都差不多的话选最右边那个
        }
    }
    if (best_idx != -1) {
...
bool find_other_coner_edge2(Mat gray_img,Point& right_edge_point,Mat& visualizeImg){//拐角的特征是这一行有下一行没
    bool is_now_white = false;
    vector<Point> maybe_right_point;

    // 左部寻找起点
    for (int i = 269; i > 2; i--) {
        if (is_now_white && gray_img.at<uchar>(i, 370) == 0) {
            is_now_white = false;
        }
        if (!is_now_white && gray_img.at<uchar>(i, 370) == 255) {
            maybe_right_point.emplace_back(370, i);
            circle(visualizeImg, Point(370, i), 9, Scalar(255, 0, 0), -1);
            is_now_white = true;
        }
    }
    ...
    for (int idx = 0; idx < point_number; idx++) {
        ...
        if (racetracks[idx].points.size() > 60) racetracks[idx].right_point = true;
    }
    ...
    for (int i = 0; i < point_number; i++) {
        if(racetracks[i].right_point){//点要足够多
            float direction_change_in_total = racetracks[i].direction_change/(float)racetracks[i].points.size();
            if (direction_change_in_total<first) {
                first = direction_change_in_total;
                best_idx = i;
...
int recently_white(Mat gray_img,Mat& visualizeImg){//回到路口的时候，只能看到正前方有白线，通过白线距离来判断怎么走
    ...
    for (int idx = 0; idx < point_number; idx++) {
        ...
        // if (racetracks[idx].points.size() > 50) racetracks[idx].right_point = true;
    }
...
public:
    LineFollowerNode() : nh(""){
        line_server = nh.advertiseService("left2", &LineFollowerNode::line_server_callback,this);
    ...
    bool line_server_callback(line_follow::line_follow::Request& req,line_follow::line_follow::Response& resp){
        ...
        while(ros::ok()){
            ...
            if(out_range){
                vector<Point> start_points = find_track_edge(gray_img, 320, 70, cropped);
                RaceTrack racetrack;  // 现在RaceTrack已声明，可正常使用
                if(left_point_start_){
                    Point left_edge_point;
                    if(find_right_edge(gray_img, left_edge_point,cropped)){
