    ...
    while(ros::ok()){
        ...
        if(!out_range && !pass_out && other_enter && !pass_enter && !out_ready){
            ...
            if(other_enter_last_conner.x != 700){
                ...
                twist.angular.z = (87-other_enter_last_conner.x)*other_enter_pointx*(other_enter_last_conner.y*0.0061+0.28);
                cmd_pub.publish(twist);
        ...
        if(pass_enter_ready){
            ...
            if(recent<90){
                ...
                twist.linear.x = 0.15;
                twist.angular.z = 0;
                // ROS_INFO("回到路口前进部分%d",recent);
            }
            else{
                twist.linear.x = 0;
                twist.angular.z = -0.6;
                // waitKey(0);
                // ROS_INFO("回到路口转弯部分%d",recent);
            }
            ...
            if(right_forward){
                if(point_forward){
                    ...
                    if(pass_out){
                        ...
                        twist.linear.x = (205-right_edge_point.y)*other_enter_pointx+0.05;
                        twist.angular.z = (87-right_edge_point.x)*other_enter_pointy+0.1;
