// 版本：右线巡线 + 三段式避障 + 动态目标YAML直出 V20（2026-08-20）
// 唯一校验标识：LINE2O_RIGHT_FIXED_AVOID_TARGET_YAML_LOG_V20
// 修改基线：V15；避障点位置不变，目标航向改为指向YAML参考点，
// 终点停靠平移全程同步将车头调整至-90°（由现有参数配置）。
// 每次巡线服务启动前先向 /initialpose 发布指定AMCL初始位姿；
// 巡线满足定位触发条件后，不发布零速度，直接无缝切入终点纯PP位姿停靠。

// 2026-08-19：普通丢线前5帧只线性衰减角速度，线速度不改；第6帧固定右转及其余逻辑保持原样。
...
private:
    ...
    // 图像局部自适应二值化参数（由 line2o_right.yaml 配置）
...
public:
    ...
    LineFollowerNode() : 
        ...
        ac_(nullptr),
        output_file_("/home/ucar/ucar_ws_copy/src/line_follow/image/line2o_right.avi"),
        ...
        ROS_INFO(
            "启动 line2o_right V17（挡板近区禁止丢线右转 + 原V16其余逻辑不变）");

        // 1. 初始化服务端（优先初始化）
        line_server_ = nh_.advertiseService("line2o_right", &LineFollowerNode::line_server_callback, this);
        ROS_INFO("line2o_right服务已初始化");
...
private:
    // 加载ROS参数
    void loadParameters() {
        // 参数键名与另外两个巡线程序保持一致，但从 /line2o_right
        // 参数树读取独立的 line2o_right.yaml。
        nh_.getParam("/line2o_right/right_P", p_);
        nh_.getParam("/line2o_right/right_I", i_);
        nh_.getParam("/line2o_right/right_D", d_);
        nh_.getParam("/line2o_right/leftpoint_p", leftpoint_p_);
        nh_.getParam("/line2o_right/leftpoint_I", leftpoint_I_);
        nh_.getParam("/line2o_right/leftpoint_D", leftpoint_D_);
        nh_.getParam("/line2o_right/x_max_", x_max_);
        nh_.getParam("/line2o_right/integration_limit", integration_limit_);
        nh_.getParam("/line2o_right/out_forward", out_forward_);
        nh_.getParam("/line2o_right/out_turn", out_turn_);
        nh_.getParam("/line2o_right/out_turn_angel", out_turn_angel_);
        nh_.getParam("/line2o_right/center_distance", center_distance);

        // 从 line2o_right.yaml 读取图像二值化参数。
        // YAML 中未填写时，默认值与当前实车参数保持一致：45、-22、250。
        nh_.param("/line2o_right/adaptive_block", adaptive_block_, 45);
        nh_.param("/line2o_right/adaptive_c", adaptive_c_, -22);
        nh_.param("/line2o_right/min_contour_area", min_contour_area_, 250);

        // 雷达挡板检测：遍历整帧雷达，利用map场地边界排除四周墙面。
        nh_.param<string>("/line2o_right/scan_topic", scan_topic_, "/scan");
        nh_.param("/line2o_right/barrier_min_scan_range",
                  barrier_min_scan_range_, 0.05);
        nh_.param("/line2o_right/barrier_max_scan_range",
                  barrier_max_scan_range_, 2.00);
        nh_.param("/line2o_right/field_min_x", field_min_x_, 0.0);
        nh_.param("/line2o_right/field_max_x", field_max_x_, 5.0);
        nh_.param("/line2o_right/field_min_y", field_min_y_, 0.0);
        nh_.param("/line2o_right/field_max_y", field_max_y_, 2.5);
        nh_.param("/line2o_right/barrier_boundary_margin",
                  barrier_boundary_margin_, 0.10);
        nh_.param("/line2o_right/barrier_candidate_max_map_x",
                  barrier_candidate_max_map_x_, 2.0);
        nh_.param("/line2o_right/barrier_neighbor_max_distance",
                  barrier_neighbor_max_distance_, 0.10);
        nh_.param("/line2o_right/barrier_min_points",
                  barrier_min_points_, 6);
        nh_.param("/line2o_right/barrier_min_line_length",
                  barrier_min_line_length_, 0.20);
        nh_.param("/line2o_right/barrier_trigger_zone_min_line_length",
                  barrier_trigger_zone_min_line_length_, 0.005);
        nh_.param("/line2o_right/barrier_max_rms_error",
                  barrier_max_rms_error_, 0.025);
        nh_.param("/line2o_right/barrier_endpoint_margin",
                  barrier_endpoint_margin_, 0.05);
        nh_.param("/line2o_right/barrier_trigger_distance",
                  barrier_trigger_distance_, 0.20);
        // 挡板近区丢线转向抑制：YAML可显式配置；未配置时默认0.35m。
        nh_.param("/line2o_right/barrier_no_drift_distance",
                  barrier_no_drift_distance_, 0.35);
        nh_.param("/line2o_right/barrier_slowdown_start_distance",
                  barrier_slowdown_start_distance_, 1.00);
        nh_.param("/line2o_right/barrier_slowdown_min_speed_ratio",
                  barrier_slowdown_min_speed_ratio_, 0.50);
        nh_.param("/line2o_right/barrier_confirm_scans",
                  barrier_confirm_scans_, 1);
        nh_.param("/line2o_right/barrier_confirm_max_distance_change",
                  barrier_confirm_max_distance_change_, 0.08);
        nh_.param("/line2o_right/barrier_confirm_max_angle_change_deg",
                  barrier_confirm_max_angle_change_deg_, 15.0);
        nh_.param("/line2o_right/barrier_debug_log_enabled",
                  barrier_debug_log_enabled_, true);
        nh_.param("/line2o_right/barrier_debug_log_interval",
                  barrier_debug_log_interval_, 0.50);
        nh_.param("/line2o_right/barrier_scan_timeout",
                  barrier_scan_timeout_, 1.00);

        // 锁定挡板后：沿板向左、垂直板向前、沿板向右。
        nh_.param("/line2o_right/avoid_left_distance",
                  avoid_left_distance_, 0.50);
        nh_.param("/line2o_right/avoid_forward_distance",
                  avoid_forward_distance_, 0.50);
        nh_.param("/line2o_right/avoid_right_distance",
                  avoid_right_distance_, 0.50);
        nh_.param("/line2o_right/avoid_left_speed",
                  avoid_left_speed_, 0.35);
        nh_.param("/line2o_right/avoid_forward_speed",
                  avoid_forward_speed_, 0.45);
        nh_.param("/line2o_right/avoid_right_speed",
                  avoid_right_speed_, 0.35);
        nh_.param("/line2o_right/avoid_control_rate",
                  avoid_control_rate_, 40.0);
        nh_.param("/line2o_right/avoid_position_kp",
                  avoid_position_kp_, 2.0);
        nh_.param("/line2o_right/avoid_yaw_kp",
                  avoid_yaw_kp_, 2.5);
        nh_.param("/line2o_right/avoid_max_angular_speed",
                  avoid_max_angular_speed_, 0.60);
        nh_.param("/line2o_right/avoid_rotate_min_angular_speed",
                  avoid_rotate_min_angular_speed_, 0.10);
        nh_.param("/line2o_right/avoid_rotate_tolerance_deg",
                  avoid_rotate_tolerance_deg_, 2.0);
        nh_.param("/line2o_right/avoid_rotate_timeout",
                  avoid_rotate_timeout_, 3.0);
        nh_.param("/line2o_right/avoid_heading_pause_error_deg",
                  avoid_heading_pause_error_deg_, 6.0);
        nh_.param("/line2o_right/avoid_position_tolerance",
                  avoid_position_tolerance_, 0.02);
        nh_.param("/line2o_right/avoid_min_linear_speed",
                  avoid_min_linear_speed_, 0.08);
        nh_.param("/line2o_right/avoid_segment_timeout",
                  avoid_segment_timeout_, 5.0);
        nh_.param("/line2o_right/avoid_stop_hold_time",
                  avoid_stop_hold_time_, 0.10);
        nh_.param("/line2o_right/avoid_segment_pause_time",
                  avoid_segment_pause_time_, 0.05);
        nh_.param("/line2o_right/avoid_acc_lim_x",
                  avoid_acc_lim_x_, 2.0);
        nh_.param("/line2o_right/avoid_acc_lim_y",
                  avoid_acc_lim_y_, 2.0);
        nh_.param("/line2o_right/avoid_acc_lim_theta",
                  avoid_acc_lim_theta_, 6.0);
        nh_.param("/line2o_right/avoid_camera_flush_frames",
                  avoid_camera_flush_frames_, 5);
        nh_.param("/line2o_right/avoid_move_base_extension_distance",
                  avoid_move_base_extension_distance_, 0.30);
        nh_.param("/line2o_right/avoid_move_base_parallel_shift_distance",
                  avoid_move_base_parallel_shift_distance_, 0.0);
        nh_.param("/line2o_right/avoid_move_base_heading_target_x",
                  avoid_move_base_heading_target_x_, 0.20);
        nh_.param("/line2o_right/avoid_move_base_heading_target_y",
                  avoid_move_base_heading_target_y_, 0.50);
        nh_.param("/line2o_right/avoid_move_base_min_target_x",
                  avoid_move_base_min_target_x_, 0.0);
        nh_.param("/line2o_right/avoid_use_fixed_target",
                  avoid_use_fixed_target_, false);
        nh_.param("/line2o_right/avoid_fixed_target_x",
                  avoid_fixed_target_x_, 0.0);
        nh_.param("/line2o_right/avoid_fixed_target_y",
                  avoid_fixed_target_y_, 0.0);
        nh_.param("/line2o_right/avoid_fixed_target_yaw_deg",
                  avoid_fixed_target_yaw_deg_, -90.0);
        nh_.param("/line2o_right/avoid_move_base_timeout",
                  avoid_move_base_timeout_, 10.0);
        nh_.param("/line2o_right/avoid_recenter_y_kp",
                  avoid_recenter_y_kp_, 0.004);
        nh_.param("/line2o_right/avoid_recenter_min_y_speed",
                  avoid_recenter_min_y_speed_, 0.06);
        nh_.param("/line2o_right/avoid_recenter_max_y_speed",
                  avoid_recenter_max_y_speed_, 0.30);
        nh_.param("/line2o_right/avoid_recenter_line_error_tolerance",
                  avoid_recenter_line_error_tolerance_, 8.0);
        nh_.param("/line2o_right/avoid_recenter_confirm_frames",
                  avoid_recenter_confirm_frames_, 5);
        nh_.param("/line2o_right/avoid_recenter_timeout",
                  avoid_recenter_timeout_, 5.0);

        // AMCL初始位姿。每次服务真正启动巡线前都会重新读取并发布。
        nh_.param<string>("/line2o_right/map_frame", map_frame_, "map");
        nh_.param<string>("/line2o_right/base_frame", base_frame_, "base_link");
        nh_.param("/line2o_right/initial_pose_x", initial_pose_x_, 2.50);
        nh_.param("/line2o_right/initial_pose_y", initial_pose_y_, 2.60);
        nh_.param("/line2o_right/initial_pose_yaw_deg", initial_pose_yaw_deg_, -90.0);
        nh_.param("/line2o_right/initial_pose_covariance_xy",
                  initial_pose_covariance_xy_, 0.01);
        nh_.param("/line2o_right/initial_pose_covariance_yaw",
                  initial_pose_covariance_yaw_, 0.01);
        nh_.param("/line2o_right/initial_pose_publish_count",
                  initial_pose_publish_count_, 3);
        nh_.param("/line2o_right/initial_pose_publish_interval",
                  initial_pose_publish_interval_, 0.10);

        // 巡线终止触发条件与固定终点。
        nh_.param("/line2o_right/docking_trigger_max_x",
                  docking_trigger_max_x_, 0.75);
        nh_.param("/line2o_right/docking_trigger_distance",
                  docking_trigger_distance_, 0.75);
        nh_.param("/line2o_right/docking_require_avoidance_complete",
                  docking_require_avoidance_complete_, true);
        nh_.param("/line2o_right/docking_goal_x", docking_goal_x_, 0.20);
        nh_.param("/line2o_right/docking_goal_y", docking_goal_y_, 0.25);
        nh_.param("/line2o_right/docking_goal_yaw_deg",
                  docking_goal_yaw_deg_, -90.0);

        // 终点纯PP位姿控制，参数默认照搬局部规划器原最终姿态调整。
        nh_.param("/line2o_right/docking_control_rate",
                  docking_control_rate_, 30.0);
        nh_.param("/line2o_right/docking_position_tolerance",
                  docking_position_tolerance_, 0.025);
        nh_.param("/line2o_right/docking_yaw_tolerance",
                  docking_yaw_tolerance_, 0.05);
        nh_.param("/line2o_right/docking_relaxed_yaw_tolerance",
                  docking_relaxed_yaw_tolerance_, 0.08);
        nh_.param("/line2o_right/docking_relaxed_hold_time",
                  docking_relaxed_hold_time_, 0.50);
        nh_.param("/line2o_right/docking_timeout",
                  docking_timeout_, 12.0);
        nh_.param("/line2o_right/docking_linear_x_gain",
                  docking_linear_x_gain_, 2.50);
        nh_.param("/line2o_right/docking_linear_y_gain",
                  docking_linear_y_gain_, 1.20);
        nh_.param("/line2o_right/docking_angular_gain",
                  docking_angular_gain_, 1.50);
        nh_.param("/line2o_right/docking_min_linear_speed",
                  docking_min_linear_speed_, 0.10);
        nh_.param("/line2o_right/docking_min_angular_speed",
                  docking_min_angular_speed_, 0.010);
        nh_.param("/line2o_right/docking_max_vel_x",
                  docking_max_vel_x_, 0.90);
        nh_.param("/line2o_right/docking_max_vel_y",
                  docking_max_vel_y_, 0.40);
        nh_.param("/line2o_right/docking_max_vel_theta",
                  docking_max_vel_theta_, 0.90);
        nh_.param("/line2o_right/docking_acc_lim_x",
                  docking_acc_lim_x_, 2.00);
        nh_.param("/line2o_right/docking_acc_lim_y",
                  docking_acc_lim_y_, 2.00);
        nh_.param("/line2o_right/docking_acc_lim_theta",
        ...
        ROS_INFO(
            "一次性绕障：先沿板左移%.3fm（限速%.3fm/s），"
            "再从锁存板中点沿前向板法向延伸%.3fm，"
            "并沿板平移%.3fm（正值默认向map_x增大方向），"
            "目标航向指向参考点(%.3f, %.3f)，"
    ...
    bool line_server_callback(line_follow::line_follow::Request& req, line_follow::line_follow::Response& resp) {
        ...
        ROS_INFO("line2o_right巡线启动：挡板检测已启用，本次服务最多触发一次。");
        ...
        while (ros::ok()) {
            ...
            if (getRobotPose(robot_x, robot_y, robot_yaw)) {
                ...
                const bool inside_docking_trigger =
                    robot_x < docking_trigger_max_x_ &&
    ...
    void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan) {
        ...
        for (size_t index = 0; index < scan->ranges.size(); ++index) {
            ...
            if (point_map_x >= barrier_candidate_max_map_x_) {
                continue;
            }
        ...
        if (barrier_debug_log_enabled_) {
            if (nearest_observed_fit.computed) {
                ROS_INFO_THROTTLE(
                    ...
                    barrier_candidate_max_map_x_, x_region_point_count,
    ...
    bool runOneTimeAvoidance() {
        ...
        if (avoid_use_fixed_target_) {
            ...
            ROS_WARN(
                "========== 本次动态避障目标【可直接写入line2o_right.yaml】 ==========\n"
        ...
        ROS_WARN(
            ...
            final_target_yaw * 180.0 / 3.14159265358979323846,
            final_target_source,
            "第一段沿板左移", avoid_left_distance_, avoid_forward_distance_);

        const bool left_ok = executeAvoidanceSegment(
            "第一段沿板左移",
            locked_fit.direction_x, locked_fit.direction_y,
            avoid_left_distance_, avoid_left_speed_,
            locked_yaw, locked_yaw);
        if (left_ok && avoid_segment_pause_time_ > 0.0) {
            ros::WallDuration(avoid_segment_pause_time_).sleep();
        }

        const bool forward_ok = left_ok && executeAvoidanceSegment(
            "第二段垂直板向前平移",
            locked_fit.normal_x, locked_fit.normal_y,
        ...
        lost_start_angular_z_ = 0.0;
        ROS_WARN(
            "第一段沿板左移、第二段垂直板前移和第三段最终位姿调整均已完成；挡板检测永久关闭并恢复line2o_right正常巡线。");
        return true;
    }
    ...
    void runLeftPointTracking(Mat& gray_img, Mat& cropped) {
        ...
        if (!point_forward_) {
            ...
            if (pose_.response.pose_at[2] < out_turn_angel_) {
                ...
                double_line_ = true;
                x_max_ = 0.5;
                nh_.getParam("/line2o_right/double_P", p_);
                nh_.getParam("/line2o_right/double_I", i_);
                nh_.getParam("/line2o_right/double_D", d_);
    ...
    void runNormalTracking(Mat& gray_img, Mat& cropped) {
        ...
        lost_start_angular_z_ = 0.0;
            ...
            if (trace_failed_count_ > 5) {
                ...
                // V17新增：挡板进入barrier_no_drift_distance后，到本次避障结束前，
                // 即使视觉连续丢线也只保持低速直行，不允许触发固定右转。
                // 避障完成后barrier_no_drift_active_会清零，原固定右转逻辑自动恢复。
                if (barrier_no_drift_active_.load() &&
                    ...
                    // 原V16逻辑：ROS约定 angular.z < 0 为顺时针，即向右转。
...
int main(int argc, char **argv) {
    setlocale(LC_ALL, "");
    ros::init(argc, argv, "line2o_right");
