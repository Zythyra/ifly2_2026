// 版本：lineo_right V30基础 + 终点停靠防死区 V31（2026-08-19）
// 唯一校验标识：LINEO_RIGHT_DOCKING_DEADZONE_FIX_V11
...
private:
    ...
    void loadParameters() {
        ...
        nh_.param("/lineo_right/docking_relaxed_yaw_tolerance",
                  docking_relaxed_yaw_tolerance_, 0.08);
        nh_.param("/lineo_right/docking_relaxed_hold_time",
                  docking_relaxed_hold_time_, 0.50);
        nh_.param("/lineo_right/docking_timeout",
    ...
    bool runDockingControl() {
        ...
        // 与之前已经验证的版本相同的终点停靠防死区机制：
        // 1. 严格位置+方向容差满足时立即正常完成；
        // 2. 位置已经到位，方向进入稍宽松容差并连续保持一段时间，也判定完成；
        // 3. 设置停靠总超时，避免静摩擦/AMCL抖动导致小速度永久输出。
