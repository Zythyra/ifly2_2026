// 版本：寻右线 + 启动视觉屏蔽定向直行 + 渐进丢线判定 + 锁存全向甩尾软退出 + AMCL定位/停靠版 V14（2026-08-07）
...
public:
    ...
    LineFollowerNode() : 
        ...
        ROS_INFO("启动 line_right V13（启动定向直行、渐进丢线判定、锁存甩尾、AMCL纯PP停靠）");
...
private:
    ...
    void loadParameters() {
        ...
        nh_.param("/line_right/start_straight_speed", start_straight_speed_, 0.50);
        nh_.param("/line_right/start_straight_distance", start_straight_distance_, 0.20);
        nh_.param("/line_right/start_heading_yaw_deg", start_heading_yaw_deg_, -90.0);
        nh_.param("/line_right/start_heading_kp", start_heading_kp_, 2.5);
        nh_.param("/line_right/start_heading_max_angular_speed",
                  start_heading_max_angular_speed_, 0.60);
        nh_.param("/line_right/start_heading_deadband_deg",
                  start_heading_deadband_deg_, 0.5);
        nh_.param("/line_right/start_heading_pause_error_deg",
                  start_heading_pause_error_deg_, 6.0);
        nh_.param("/line_right/start_control_rate", start_control_rate_, 30.0);
        nh_.param("/line_right/start_line_confirm_frames",
        ...
        nh_.param("/line_right/out_left_speed", out_left_speed_, 0.16);
        nh_.getParam("/line_right/out_turn", out_turn_);
        // 丢线状态机参数。疑似丢线阶段的vx由x_max_与out_forward_自动线性插值。
        nh_.param("/line_right/lost_confirm_frames", lost_confirm_frames_, 5);
        nh_.param("/line_right/lost_cancel_confirm_frames", lost_cancel_confirm_frames_, 2);
        nh_.param("/line_right/drift_recover_confirm_frames", drift_recover_confirm_frames_, 2);
        nh_.param("/line_right/lost_confirm_linear_y", lost_confirm_linear_y_, 0.0);
        nh_.param("/line_right/lost_confirm_angular_z", lost_confirm_angular_z_, 0.0);
        ...
        nh_.param<string>("/line_right/map_frame", map_frame_, "map");
        nh_.param<string>("/line_right/base_frame", base_frame_, "base_link");
        nh_.param("/line_right/initial_pose_x", initial_pose_x_, 2.50);
        nh_.param("/line_right/initial_pose_y", initial_pose_y_, 2.60);
        nh_.param("/line_right/initial_pose_yaw_deg", initial_pose_yaw_deg_, -90.0);
        nh_.param("/line_right/initial_pose_covariance_xy",
                  initial_pose_covariance_xy_, 0.01);
        nh_.param("/line_right/initial_pose_covariance_yaw",
                  initial_pose_covariance_yaw_, 0.01);
        nh_.param("/line_right/initial_pose_publish_count",
                  initial_pose_publish_count_, 3);
        nh_.param("/line_right/initial_pose_publish_interval",
                  initial_pose_publish_interval_, 0.10);

        // 新版停靠触发：y小于阈值且距离固定终点足够近。
        nh_.param("/line_right/docking_trigger_max_y",
                  docking_trigger_max_y_, 0.75);
        nh_.param("/line_right/docking_trigger_distance",
                  docking_trigger_distance_, 0.75);
        nh_.param("/line_right/docking_goal_x", docking_goal_x_, 0.25);
        nh_.param("/line_right/docking_goal_y", docking_goal_y_, 0.25);
        nh_.param("/line_right/docking_goal_yaw_deg",
                  docking_goal_yaw_deg_, 180.0);

        // 终点纯PP位姿控制。
        nh_.param("/line_right/docking_control_rate",
                  docking_control_rate_, 30.0);
        nh_.param("/line_right/docking_position_tolerance",
        ...
        nh_.param("/line_right/docking_angular_gain",
                  docking_angular_gain_, 1.50);
        nh_.param("/line_right/docking_min_linear_speed",
                  docking_min_linear_speed_, 0.10);
        nh_.param("/line_right/docking_min_angular_speed",
                  docking_min_angular_speed_, 0.010);
        nh_.param("/line_right/docking_max_vel_x",
                  docking_max_vel_x_, 0.90);
        nh_.param("/line_right/docking_max_vel_y",
                  docking_max_vel_y_, 0.40);
        nh_.param("/line_right/docking_max_vel_theta",
                  docking_max_vel_theta_, 0.90);
        nh_.param("/line_right/docking_acc_lim_x",
                  docking_acc_lim_x_, 2.00);
        nh_.param("/line_right/docking_acc_lim_y",
                  docking_acc_lim_y_, 2.00);
        nh_.param("/line_right/docking_acc_lim_theta",
        ...
        ROS_INFO("line_right参数加载完成：center_distance=%d", center_distance);
        ROS_INFO(
    ...
    bool line_server_callback(line_follow::line_follow::Request& req, line_follow::line_follow::Response& resp) {
        ...
        while (ros::ok()) {
            ...
            if (initial_straight_mode) {
                ...
                const bool found_right_line = runNormalTracking(gray_img, cropped, false);
