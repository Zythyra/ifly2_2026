...
double trace_edge(Point start_point, Mat& gray_img, vector<Point>& traced_points, bool& right,  
    ...
    double line_slope = lineParams[1]/lineParams[0];
    if((line_slope<0.05&&line_slope>-10)||traced_points.size()<15){//不接受右线向右倾斜数量太少不要
        right = false;
    }
    // 可视化追踪过程
